...
class JavaExample {
	...
	public void enumExample(Context ctx) throws TestFailedException {
		...
		disprove(ctx, (BoolExpr) ctx.mkApp(fruit.getTesterDecls()[0], orange),
				false);
		prove(ctx, ctx.mkNot((BoolExpr) ctx.mkApp(fruit.getTesterDecls()[0],
				orange)), false);
	...
	public void evalExample2(Context ctx) {
		...
		System.err.println((solver.getAssertions()[0].getFuncDecl()));
		System.err.println(solver.getAssertions()[0]);
		System.err.println(solver.getAssertions()[1]);
	...
	public void floatingPointExample1(Context ctx) throws TestFailedException {
		...
		FPNum z = (FPNum) ctx.mkNumeral("-1.25p3", s); /*
														 ...
														 * 8 = -10
														 */
		System.out.println("x=" + x.toString() + "; y=" + y.toString() + "; z="
				+ z.toString());
		...
		if (slvr.check() != Status.UNSATISFIABLE)
			throw new TestFailedException();
		System.out.println("OK, unsat:" + System.getProperty("line.separator")
				+ slvr);
		...
		if (slvr.check() != Status.SATISFIABLE)
			throw new TestFailedException();
		System.out.println("OK, sat:" + System.getProperty("line.separator")
				+ slvr);
		...
		slvr.add(ctx.mkEq(x_plus_y,
				ctx.mkFPMul(ctx.mkFPRoundNearestTiesToAway(), x, y)));
	...
	public void floatingPointExample2(Context ctx) throws TestFailedException {
		...
		BitVecExpr x = (BitVecExpr) ctx.mkConst(ctx.mkSymbol("x"),
				ctx.mkBitVecSort(64));
		...
		BoolExpr c4 = ctx.mkEq(ctx.mkNumeral(42, ctx.getRealSort()),
	...
	public void optimizeExample(Context ctx) {
		...
		opt.Add(ctx.mkEq(ctx.mkAdd(xExp, yExp), ctx.mkInt(10)),
				ctx.mkGe(xExp, ctx.mkInt(0)), ctx.mkGe(yExp, ctx.mkInt(0)));
